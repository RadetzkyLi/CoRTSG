...
from coriskyscene.data_collection.sumo_integration.sumo_simulation import SumoSignalState, SumoVehSignal
...
class BridgeHelper(object):
    ...
    _vtypes_path = os.path.join(os.path.dirname(os.path.realpath(__file__)),
                                "vtypes.json")
    ...
    @staticmethod
    def get_carla_velocity(sumo_velocity):
        """
        Returns carla velocity based on sumo speed.
        
        Parameters
        ----------
        sumo_velocity: List[float]
            Actor's longitudinal and lateral speed retrieved from SUMO.

        Returns
        -------
        carla_velocity: CARLA.Vector3D
            Actor's velocity.

        Notes
        -----
        1. To comply with SUMO, we ignore speed in vertical direction, i.e.,
            only speed in x-y plane.
        """
        carla_velocity = carla.Vector3D(sumo_velocity[0], sumo_velocity[1], 0)
        return carla_velocity
    
    @staticmethod
    def get_sumo_velocity(carla_velocity):
        """
        Returns sumo speed based on carla velocity.
        """
        sumo_velocity = [carla_velocity.x, carla_velocity.y]
        return sumo_velocity
    ...
    def _create_sumo_vtype(carla_actor):
        ...
        if BridgeHelper.is_walker(carla_actor):
    ...
    def get_sumo_vtype(carla_actor):
        ...
        if not BridgeHelper.is_vehicle(carla_actor) and\
            not BridgeHelper.is_walker(carla_actor):
            logging.error(
                '[BridgeHelper] Blueprint %s not supported. No vehicle/person will be spawned in sumo',
                type_id)
            return None
    ...
    def is_vehicle(actor):
        """Return True if the given CARLA actor is a vehicle.
        """
    ...
    @staticmethod
    def is_walker(actor):
        """Return True if the given CARLA actor is a walker."""
