...
from .constants import INVALID_ACTOR_ID,CustomActorType
...
class SumoSimulation(object):
    ...
    def __init__(self, cfg_file, step_length, host=None, port=None, sumo_gui=False, client_order=1):
        ...
        # Structures to keep track of the spawned and destroyed persons at each time step.
    ...
    def subscribe(actor_id):
        ...
        if actor_id in traci.vehicle.getIDList():
            traci.vehicle.subscribe(actor_id, config)
        elif actor_id in traci.person.getIDList():
            config.pop()
            config.pop()
            traci.person.subscribe(actor_id, config)
        else:
            raise ValueError("actor_id {0} doesn't belong to vehicle nor person!".format(actor_id))
    ...
    @staticmethod
    def create_actor_from_subs(results, is_vehicle=True):
        """Create actor (vehicle or person) from subscription results.
        ...
        is_vehicle: bool
            True if the actor is vehicle.Otherwise, the actor is person.
        ...
        speed_lon = results[traci.constants.VAR_SPEED]

        if is_vehicle:
            signals = results[traci.constants.VAR_SIGNALS]
            speed_lat = results[traci.constants.VAR_SPEED_LAT]
            speed = [speed_lon, speed_lat]
            transform = carla.Transform(carla.Location(location[0], location[1], location[2]),
                                        carla.Rotation(rotation[0], rotation[1], rotation[2]))
        else:
            signals = None
            speed = [speed_lon, 0]
            transform = carla.Transform(carla.Location(location[0], location[1], location[2]+extent.z), 
                                        carla.Rotation(rotation[0], rotation[1], rotation[2]))
    ...
    def get_actor(actor_id):
        ...
        if actor_id in traci.vehicle.getIDList():
            ...
            return SumoSimulation.create_actor_from_subs(results)
        elif actor_id in traci.person.getIDList():
            results = traci.person.getSubscriptionResults(actor_id)
            return SumoSimulation.create_actor_from_subs(results, is_vehicle=False)
        else:
            raise ValueError("actor_id {0} doesn't belong to vehicle nor person!".format(actor_id))
    
    @staticmethod
    def get_custom_actor_type(actor_id):
        """
        Get custom actor type based on given sumo object id

        Notes: vehicle id and person id can have overlapping, so, to let the following code work, one
            should use different naming rules when creating id for vehicle and person.
        """
        if actor_id in traci.vehicle.getIDList():
            ...
            raise ValueError("Actor id %s is not in spawned actors!"%(actor_id))
        
    def is_vehicle(self, actor_id):
        if actor_id in self.spawned_actors-self.spawned_persons:
            return True
        return False
    ...
    def spawn_actor(self, type_id, color=None):
        ...
        try:
            ...
            if vclass == 'pedestrian':
                # TODO: spawn pedestrians to sumo
                pass
            else:
    ...
    def destroy_actor(actor_id):
        ...
        """
        if actor_id in traci.person.getIDList():
            traci.person.remove(actor_id)
        elif actor_id in traci.vehicle.getIDList():
            traci.vehicle.remove(actor_id)
        else:
            ValueError("actor_id {0} doesn't belong to vehicle nor person!".format(actor_id))
    ...
    def synchronize_actor(self, actor_id, transform, signals=None):
        """Update vehicle/person state.
        
        Parameters
        ----------
        vehicle_id: str
            Id of sumo actor to be synchronized.
        
        transform: CARLA.transform
            Target vehicle pose.
        
        signals: CARLA.VehicleLightState
            Vehicle's light state
        ...
        if actor_id in traci.person.getIDList():
            traci.person.moveToXY(actor_id, "", loc_x, loc_y, angle=yaw, keepRoute=2)
        elif actor_id in traci.vehicle.getIDList():
            traci.vehicle.moveToXY(actor_id, "", 0, loc_x, loc_y, angle=yaw, keepRoute=2)
            if signals is not None:
                traci.vehicle.setSignals(actor_id, signals)
        else:
            raise ValueError("actor_id {0} doesn't belong to vehicle nor person!".format(actor_id))
        return True
    ...
    def synchronize_vehicle(self, vehicle_id, transform, signals=None):
        ...
        Updates vehicle state.
            ...
            :param speed: new vehicle speed in longitudinal and lateral direction.
    ...
    def tick(self):
        ...
        self.spawned_actors = set(traci.simulation.getDepartedIDList()) | self.spawned_persons
        self.destroyed_actors = set(traci.simulation.getArrivedIDList()) | self.destroyed_persons

        self.spawned_vehicles = traci.simulation.getDepartedIDList()
