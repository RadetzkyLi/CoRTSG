...
import abc
import numpy as np
from coriskyscene.opendriveparser.elements.eulerspiral import EulerSpiral
...
class Geometry(abc.ABC):
    ...
    def __init__(self, start_position, heading: float, length: float):
        # start_postion: array-like, e.g., [2.0, 4.1]
    ...
    def calc_positions(self, s_pos_array):
        """This function should be overwrited if we can do batch
        calculation for specific functions.
        
        Args:
            s_pos_array: {array-like}, 1d

        Returns:
            pos_array: {array-like}, (n,2)
            tangent_array: {array-like}, (n,)
        """
        n = len(s_pos_array)
        pos_array = np.zeros((n,2))
        tangent_array = np.zeros((n,))
        for i,s_pos in enumerate(s_pos_array):
            pos,tangent = self.calc_position(s_pos)
            pos_array[i] = pos
            tangent_array[i] = tangent
        return (pos_array, tangent_array)
...
class Line(Geometry):
    ...
    def calc_positions(self, s_pos_array):
        """Calculate position for many s_pos at a time.
        
        Args:
            s_pos_list: {array-like}
        
        Returns:
            pos_array: {ndarray}, (n,2)
            tangent_array: {ndarray}, (n,)

        """
        c_h = np.cos(self.heading)
        s_h = np.sin(self.heading)
        s_pos_array = np.array(s_pos_array)
        n = len(s_pos_array)
        pos_array = self.start_position + np.hstack((
            np.reshape(s_pos_array*c_h, (n,1)),
            np.reshape(s_pos_array*s_h, (n,1))
        ))
        tangent_array = np.ones((n,))*self.heading
        return (pos_array, tangent_array)
...
class Arc(Geometry):
    ...
    def calc_positions(self, s_pos_array):
        ...
        """
        if not isinstance(s_pos_array, np.ndarray):
            s_pos_array = np.array(s_pos_array)
        n = len(s_pos_array)
        ...
        a = 2 / c * np.sin(s_pos_array * c / 2)
        alpha = (np.pi - s_pos_array * c) / 2 - hdg

        dx = -1 * a * np.cos(alpha)
        dy = a * np.sin(alpha)

        pos = self.start_position + np.hstack((
            np.reshape(dx, (n,1)),
            np.reshape(dy, (n,1))
        )) 
        tangent = self.heading + s_pos_array * self.curvature
...
class Poly3(Geometry):
    ...
    @staticmethod
    def calc_function_value(ds: float, polynomial_coefficients):
        """
        
        Args:
            ds: 
            polynomial_coeffcients: array-like, standing for [a, b, c, d]

        Returns:
            v: float, a+b*ds+c*ds**2+d*ds**3
        """
        v = polynomial_coefficients[0] +\
            polynomial_coefficients[1] * ds +\
            polynomial_coefficients[2] * ds**2 +\
            polynomial_coefficients[3] * ds**3
        return v
