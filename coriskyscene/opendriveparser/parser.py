...
from coriskyscene.opendriveparser.elements.opendrive import OpenDrive, Header
from coriskyscene.opendriveparser.elements.road import Road
from coriskyscene.opendriveparser.elements.roadLink import (
    Predecessor as RoadLinkPredecessor,
    Successor as RoadLinkSuccessor,
    Neighbor as RoadLinkNeighbor,
)
from coriskyscene.opendriveparser.elements.roadtype import (
    RoadType,
...
from coriskyscene.opendriveparser.elements.roadLateralProfile import (
...
from coriskyscene.opendriveparser.elements.roadLanes import (
...
from coriskyscene.opendriveparser.elements.junction import (
    Junction,
    Connection as JunctionConnection,
    LaneLink as JunctionConnectionLaneLink,
)
from coriskyscene.opendriveparser.elements.roadObjects import (
    RoadObjects,
    RoadOjbect
)
...
def parse_opendrive_road_elevation_profile(newRoad, road_elevation_profile):
    ...
    for elevation in road_elevation_profile.findall("elevation"):
        ...
        newRoad.elevationProfile.addElevationRecord(
            RoadElevationProfile(
                float(elevation.get("a")),
                float(elevation.get("b")),
                float(elevation.get("c")),
    ...
    newRoad.elevationProfile.calcElevationLengths(newRoad.length)
...
def parse_opendrive_road_objects(newRoad, objects):
    """
    ...
    Args:
        ...
        road
    """
    newObjects = newRoad.objects

    # currently, only parse crosswalk
    target_road_object_types = ['crosswalk']
    
    for road_object in objects.findall("object"):
        if road_object.get("type") not in target_road_object_types:
            continue

        newRoadObject = RoadOjbect()

        newRoadObject.id = road_object.get("id")
        newRoadObject.type = road_object.get("type")

        # object's pose
        newRoadObject.origin_s_pos = float(road_object.get("s"))
        newRoadObject.origin_t_pos = float(road_object.get("t"))
        newRoadObject.z_offset = float(road_object.get("zOffset"))
        newRoadObject.heading = float(road_object.get("hdg"))
        newRoadObject.pitch = float(road_object.get("pitch"))
        newRoadObject.roll = float(road_object.get("roll"))

        # bbox 
        newRoadObject.width = float(road_object.get("width"))
        newRoadObject.length = float(road_object.get("length"))

        # TODO parse other attrs

        newObjects.objects.append(newRoadObject)
...
def parse_opendrive_road(opendrive, road):
    ...
    if road_objects is not None:
        parse_opendrive_road_objects(newRoad, road_objects)
